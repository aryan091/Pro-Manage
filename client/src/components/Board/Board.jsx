...
function Board() {
  ...
  return (
    <div className="w-[75%] h-full fixed ml-[25%] p-4">
      ...
      <div className={`cards-container flex-1 overflow-x-auto ${loading ? '' : 'custom-scrollbar'}`}>
      <div className="cards flex space-x-4 w-max">
          {Object.keys(tasks).map(column => (
            <div key={column} className={`card w-[360px] h-[536px] flex flex-col p-4 bg-[#F2F2F2] border rounded-lg ${loading ? '' : 'overflow-y-auto custom-scrollbar'}`}>
      ...
      {location.pathname === '/app/dashboard/create-task' && (
        <CreateTaskModal closeModal={closeModal} status={'todo'} />
      )}
